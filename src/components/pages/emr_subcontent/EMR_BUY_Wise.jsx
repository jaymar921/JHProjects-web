...
/**
 * Paying for the full build through Wise.
 *
 * The counterpart to EMR_BUY_PayPal, and the same account Custom Enchantments
 * 3 uses, QR included. Wise takes 5% off on that page and nothing off here:
 * the pre-release price is the discount, and it is the same number whichever
 * route you take.
 */
const { payment, price } = PluginInformation;
const { wise } = payment;
const amount = `${wise.currencySymbol}${price.amount}`;
            ...
            <Step n="2" accent="lime">
              ...
              <div className="mt-3 border border-slate-700/70 bg-[rgba(0,0,0,0.45)] p-3">
                <Field
                  ...
                  value={
                    price.onSale
                      ? `${amount} (pre-release price, normally ${wise.currencySymbol}${price.regularAmount})`
                      : amount
                  }
                />
                <Field
