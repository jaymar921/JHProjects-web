...
/**
 * Paying for the full build through PayPal.
 *
 * Same five steps as Custom Enchantments 3, because it is the same developer
 * granting the same way: send the exact amount, screenshot the receipt, email
 * it with a Spigot username, and the resource is granted by hand.
 *
 * The one difference is the price. CE3 takes 3% off for PayPal; this takes
 * nothing off, because the plugin is already on its pre-release price. The
 * discount line is rendered only when there is a discount to render, so this
 * panel does not have to say "0% discount" to stay truthful.
 */
const { payment, price } = PluginInformation;
const { paypal } = payment;
const amount = `${paypal.currencySymbol}${price.amount}`;
  ...
  return (
    <div className="w-full pb-6">
      <Section>
        ...
        <div className="pt-3">
          <Note accent="ember" icon="fa-solid fa-tag">
            {price.saleNote} {payment.noDiscountNotice}
          </Note>
        </div>
      </Section>
            ...
            <Step n="3" accent="sky">
              ...
              <div className="mt-3 border border-slate-700/70 bg-[rgba(0,0,0,0.45)] p-3">
                <Field
                  ...
                  value={
                    price.onSale
                      ? `${amount} (pre-release price, normally ${paypal.currencySymbol}${price.regularAmount})`
                      : amount
                  }
                />
                <Field
