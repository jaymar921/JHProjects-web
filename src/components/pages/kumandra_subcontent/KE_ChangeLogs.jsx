...
function KE_ChangeLogs() {
  const [filteredLog, setFilter] = useState(Kumandra_Logs);
  const [input, setInput] = useState("");

  const latestVersion = Kumandra_Logs.find((log) => log.release_date);

  const doFilter = (e) => {
    setInput(e.target.value);

    const newList = Kumandra_Logs.filter((arg) => {
    ...
    <div className="w-full font-mono">
      <div className="p-4 md:p-8">
        <div className="flex flex-wrap place-items-center gap-3">
          <h1 className="pixel-font text-[11px] text-emerald-300 md:text-lg">
            Plugin Updates
          </h1>
          {latestVersion && (
        ...
        <div className="relative mt-2">
          ...
          <input
            ...
            className="w-full border border-slate-700 bg-[rgba(0,0,0,0.5)] py-2 pr-3 pl-9 text-xs text-slate-200 outline-none placeholder:text-slate-600 focus:border-emerald-400/60 md:text-sm"
            onChange={doFilter}
            value={input}
          />
        </div>
        ...
        <p className="pt-3 text-[10px] tracking-widest text-slate-500 uppercase">
          {filteredLog.length} of {Kumandra_Logs.length} releases shown
        </p>
