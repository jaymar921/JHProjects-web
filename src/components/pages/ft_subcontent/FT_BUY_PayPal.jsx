...
/**
 * Paying for Premium through PayPal.
 *
 * Same five steps as Custom Enchantments 3 and Epic Mobs Rework, because it
 * is the same developer granting the same way: send the exact amount,
 * screenshot the receipt, email it with a Spigot username, and the resource
 * is granted by hand.
 *
 * No PayPal discount for now, so the price is the same on every route. The
 * sale note is rendered only when there is a sale, so this panel never has
 * to print an empty line to stay truthful.
 */
  ...
  return (
    <div className="w-full pb-6">
      <Section>
        ...
        <div className="pt-3">
          <Note accent="green" icon="fa-solid fa-tag">
            {price.onSale ? `${price.saleNote} ` : ""}
            {payment.noDiscountNotice}
          </Note>
        </div>
      </Section>
            ...
            <Step n="3" accent="sky">
              ...
              <div className="mt-3 border border-slate-700/70 bg-[rgba(0,0,0,0.45)] p-3">
                <Field
                  ...
                  value={
                    price.onSale
                      ? `${amount} (sale price, normally ${paypal.currencySymbol}${price.regularAmount})`
                      : amount
                  }
                />
                <Field
