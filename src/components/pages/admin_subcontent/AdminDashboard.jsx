 ...
 * The dashboard.
 *
 * Four tabs. Plugins is what the running servers report; the other three are
 * the site's own traffic, and everything on them comes from one call to
 * GET /api/stats. The totals, the breakdowns and the project table are read
 * from the rolled up counters, so they cost one document read per project
 * however many events are behind them. The timeline and the "people" columns
 * are the two things a counter cannot answer, and the server runs them as
 * bounded aggregations in the same call. The raw rows are only fetched when a
 * project is opened.
 *
 * The plugins tab fetches its own numbers, so a slow aggregation on either
 * side does not hold up the other.
 */

const TABS = [
  { key: "plugins", label: "PLUGINS", icon: "fa-solid fa-server" },
  { key: "traffic", label: "TRAFFIC", icon: "fa-solid fa-chart-line" },
  { key: "audience", label: "AUDIENCE", icon: "fa-solid fa-users" },
  { key: "projects", label: "PROJECTS", icon: "fa-solid fa-table-list" },
];

const TAB_STORAGE_KEY = "jhp-admin-tab";
  ...
  return (
    <div className="min-h-screen w-full px-4 py-8 md:px-8">
      <div className="mx-auto flex max-w-6xl flex-col gap-5">
        <header className="flex flex-wrap place-items-center justify-between gap-4">
          <div>
            ...
            <p className="pt-2 text-[11px] text-slate-500">
              ...
              {updatedAt && tab !== "plugins" && ` · updated ${formatRelative(updatedAt)}`}
            </p>
          </div>
        ...
        <nav
          className="flex gap-1 overflow-x-auto border-b border-slate-800/70"
          role="tablist"
        ...
        >
          {TABS.map((entry) => (
            <button
              ...
              // index.css styles every <button> outside Tailwind's layers, so
              // the reset has to be inline.
              style={{
                border: "none",
                borderBottom:
                  tab === entry.key ? "2px solid rgb(56, 189, 248)" : "2px solid transparent",
                borderRadius: 0,
                padding: "10px 14px",
                marginBottom: -1,
              ...
              className={`pixel-font whitespace-nowrap tracking-widest transition-colors ${
                tab === entry.key ? "text-sky-200" : "text-slate-500 hover:text-slate-300"
              }`}
            >
              <i className={`${entry.icon} pr-2`} />
              {entry.label}
            </button>
          ))}
        </nav>

        {tab === "plugins" && <AdminPlugins onSignedOut={onSignedOut} />}
                ...
                {tab === "audience" && (
                  <>
                    ...
                    <div className="grid gap-4 lg:grid-cols-[3fr_2fr]">
                      ...
                      <ChartPanel title="DEVICES" subtitle="Across every project" accent="violet">
